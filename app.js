...
function toDbRecord(record) {
  return {
    ...
    origin_country: record.originCountry || null, local_production: localProductionCategory(record.country, record.originCountry) || null, retailer: record.store, protein: record.protein, temperature: record.temperature, product: record.product,
...
function fromDbRecord(record) {
  return normalizeRecord({
    id: record.id, date: record.date, manufacturer: record.manufacturer, collector: record.collector,
    channel: record.channel, city: record.city, country: record.country, currency: record.currency,
    originCountry: record.origin_country || "", localProduction: record.local_production || "", store: record.retailer, protein: record.protein, temperature: record.temperature, product: record.product,
...
function suggestedRetailMargin(protein = $("protein").value, country = selectedCountry()) { const baseMargin = ["Beef", "Lamb", "Goat"].includes(protein) ? 30 : 20; return baseMargin + (COUNTRY_VAT_RATES[country] ?? 0); }
...
function localProductionCategory(country, originCountry) { const c = (country || "").trim(), o = (originCountry || "").trim(); if (!c || !o) return ""; if (o === c) return "Local"; if (GCC_COUNTRIES.includes(o)) return "GCC"; if (AUSTRALIA_COUNTRIES.some(a => a.toLowerCase() === o.toLowerCase())) return "Australia"; if (o.toLowerCase() === "brazil" || o.toLowerCase() === "brasil") return "Brazil"; if (o.toLowerCase() === "south africa") return "South Africa"; if (AFRICA_COUNTRIES.some(a => a.toLowerCase() === o.toLowerCase())) return "Africa"; return "Others"; }
...
function renderReport() {
  ...
  if (!data.length) {
    ...
    if ($("reportLocalProduction")) $("reportLocalProduction").innerHTML = `<table class="report-table"><thead><tr><th>Product</th><th>Temp.</th><th>Local</th><th>GCC</th><th>Australia</th><th>Brazil</th><th>South Africa</th><th>Africa</th><th>Others</th></tr></thead><tbody><tr><td colspan="9">No rows available</td></tr></tbody></table>`;
  ...
  if ($("reportLocalProduction")) {
    ...
    const localProductionTotals = { Local: [], GCC: [], Australia: [], Brazil: [], "South Africa": [], Africa: [], Others: [] };
    data.forEach(r => {
      const category = localProductionCategory(r.country, r.originCountry);
      if (!category) return;
      localProductionTotals[category].push(number(r.priceKg));
      const key = `${r.product}|${r.temperature}`;
      if (!localProductionMap[key]) localProductionMap[key] = { product: r.product, temperature: r.temperature, Local: [], GCC: [], Australia: [], Brazil: [], "South Africa": [], Africa: [], Others: [] };
      localProductionMap[key][category].push(number(r.priceKg));
    });
    const localProductionRows = Object.values(localProductionMap)
      .map(item => ({ product: item.product, temperature: item.temperature, local: average(item.Local), gcc: average(item.GCC), australia: average(item.Australia), brazil: average(item.Brazil), southAfrica: average(item["South Africa"]), africa: average(item.Africa), other: average(item.Others), localCount: item.Local.length, gccCount: item.GCC.length, australiaCount: item.Australia.length, brazilCount: item.Brazil.length, southAfricaCount: item["South Africa"].length, africaCount: item.Africa.length, otherCount: item.Others.length }))
      .sort((a, b) => (b.localCount + b.gccCount + b.australiaCount + b.brazilCount + b.southAfricaCount + b.africaCount + b.otherCount) - (a.localCount + a.gccCount + a.australiaCount + a.brazilCount + a.southAfricaCount + a.africaCount + a.otherCount))
      .slice(0, 10);
    $("reportLocalProduction").innerHTML = `<div class="report-summary"><div class="report-stat"><small>Local</small><strong>${localProductionTotals["Local"].length ? money(average(localProductionTotals["Local"]), "USD") : "—"}</strong><span>${localProductionTotals["Local"].length} records</span></div><div class="report-stat"><small>GCC</small><strong>${localProductionTotals["GCC"].length ? money(average(localProductionTotals["GCC"]), "USD") : "—"}</strong><span>${localProductionTotals["GCC"].length} records</span></div><div class="report-stat"><small>Australia</small><strong>${localProductionTotals["Australia"].length ? money(average(localProductionTotals["Australia"]), "USD") : "—"}</strong><span>${localProductionTotals["Australia"].length} records</span></div><div class="report-stat"><small>Brazil</small><strong>${localProductionTotals["Brazil"].length ? money(average(localProductionTotals["Brazil"]), "USD") : "—"}</strong><span>${localProductionTotals["Brazil"].length} records</span></div><div class="report-stat"><small>South Africa</small><strong>${localProductionTotals["South Africa"].length ? money(average(localProductionTotals["South Africa"]), "USD") : "—"}</strong><span>${localProductionTotals["South Africa"].length} records</span></div><div class="report-stat"><small>Africa</small><strong>${localProductionTotals["Africa"].length ? money(average(localProductionTotals["Africa"]), "USD") : "—"}</strong><span>${localProductionTotals["Africa"].length} records</span></div><div class="report-stat"><small>Others</small><strong>${localProductionTotals["Others"].length ? money(average(localProductionTotals["Others"]), "USD") : "—"}</strong><span>${localProductionTotals["Others"].length} records</span></div></div><table class="report-table"><thead><tr><th>Product</th><th>Temp.</th><th>Local (US$)</th><th>GCC (US$)</th><th>Australia (US$)</th><th>Brazil (US$)</th><th>South Africa (US$)</th><th>Africa (US$)</th><th>Others (US$)</th></tr></thead><tbody>${localProductionRows.map(row => `<tr><td>${escapeHtml(row.product)}</td><td>${escapeHtml(row.temperature)}</td><td>${row.localCount ? money(row.local, "USD") : "—"}</td><td>${row.gccCount ? money(row.gcc, "USD") : "—"}</td><td>${row.australiaCount ? money(row.australia, "USD") : "—"}</td><td>${row.brazilCount ? money(row.brazil, "USD") : "—"}</td><td>${row.southAfricaCount ? money(row.southAfrica, "USD") : "—"}</td><td>${row.africaCount ? money(row.africa, "USD") : "—"}</td><td>${row.otherCount ? money(row.other, "USD") : "—"}</td></tr>`).join("") || `<tr><td colspan="9">No rows available</td></tr>`}</tbody></table>`;
  }
