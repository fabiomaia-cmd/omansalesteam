...
function updateCalculation() {
  const weight = calculatedWeight(), packPrice = calculatedPackPrice(), marketPrice = number($("priceKg").value) || (weight ? packPrice / weight : 0), fx = FX_TO_USD[$("currency").value] || 1, marketPriceUsd = marketPrice / fx, manual = number($("industryPrice").value), margin = number($("margin").value), industryUsd = manual || marketPriceUsd * (1 - margin / 100);
...
function recordFromForm() {
  const weight = calculatedWeight(), packPrice = calculatedPackPrice(), priceKg = number($("priceKg").value) || (weight ? packPrice / weight : 0), currency = $("currency").value, fx = FX_TO_USD[currency] || 1, priceUsdKg = priceKg / fx, manual = number($("industryPrice").value), margin = number($("margin").value);
