...
function reportFilteredRecords() {
  const protein = $("reportProtein").value, temperature = $("reportTemp").value, manufacturer = $("reportManufacturer").value, product = $("reportProduct").value;
  return filteredRecords().filter(r => (!protein || r.protein === protein) && (!temperature || r.temperature === temperature) && (!manufacturer || r.manufacturer === manufacturer) && (!product || r.product === product)).map(r => ({ ...r, priceKg: number(r.priceUsdKg) || number(r.priceKg) / (FX_TO_USD[r.currency] || 1), currency: "USD" }));
}
...
$("reportProtein").onchange = renderReport; $("reportTemp").onchange = renderReport; $("reportManufacturer").onchange = renderReport; $("reportProduct").onchange = renderReport; $("clearReportFilters").onclick = () => { ["reportProtein", "reportTemp", "reportManufacturer", "reportProduct"].forEach(id => $(id).value = ""); renderReport(); };
