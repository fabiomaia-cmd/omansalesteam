...
function openDialog(record = null, photo = null, transcript = "") {
  ...
  if (record) {
    ...
    for (const key of ["manufacturer", "collector", "channel", "store", "protein", "temperature", "weight", "packPrice", "priceKg", "priceUsdKg", "margin", "industryPrice", "currency", "fullPrice", "comboQty", "comboUnitWeight", "comboTotalPrice"]) if ($(key)) $(key).value = record[key] == null ? "" : record[key];
...
function renderReport() {
  ...
  data.forEach(r => {
    ...
    if (!productTempMap[key]) productTempMap[key] = { product: r.product, temperature: r.temperature, rsp: [], industry: [] };
  ...
  const productTempRows = Object.values(productTempMap).map(item => {
    ...
    return { product: item.product, temperature: item.temperature, minRsp, avgRsp: priceAvg, maxRsp, minInd, avgInd: indAvg, maxInd, spread: maxRsp - minRsp };
  ...
  data.forEach(r => {
    ...
    if (!retailerMap[key]) retailerMap[key] = { retailer: key, prices: [], industry: [] };
  ...
  const retailerRows = Object.values(retailerMap).map(item => {
    ...
    const marginPct = avgIndustry ? ((avgIndustry - avgRsp) / avgIndustry) * 100 : 0;
    return { retailer: item.retailer, avgRsp, avgIndustry, spread, marginPct };
  ...
  $("reportSummary").innerHTML = `<div class="report-stat"><small>Average RSP</small><strong>${money(avgPrice, currency)}</strong><span>per filtered kg</span></div><div class="report-stat"><small>Min RSP</small><strong>${money(priceMin, currency)}</strong><span>lowest observed</span></div><div class="report-stat"><small>Max RSP</small><strong>${money(priceMax, currency)}</strong><span>highest observed</span></div><div class="report-stat"><small>Average industry</small><strong>${money(avgIndustry, currency)}</strong><span>estimated sell price</span></div><div class="report-stat"><small>Min industry</small><strong>${money(industryMin, currency)}</strong><span>lowest industry</span></div><div class="report-stat"><small>Max industry</small><strong>${money(industryMax, currency)}</strong><span>highest industry</span></div><div class="report-stat"><small>Avg margin</small><strong>${Number(avgMarginPct).toFixed(1)}%</strong><span>vs. industry benchmark</span></div><div class="report-stat"><small>Price spread</small><strong>${money(marketSpread, currency)}</strong><span>${Number(pricingGapPct).toFixed(1)}% market delta</span></div><div class="report-stat"><small>Best value retailer</small><strong>${escapeHtml(topValueRetailer ? topValueRetailer.retailer : "—")}</strong><span>${topValueRetailer ? money(topValueRetailer.avgRsp, currency) : "No retailer"} avg RSP</span></div>`;

  $("reportProductTemp").innerHTML = `<table class="report-table"><thead><tr><th>Product</th><th>Temp.</th><th>Min RSP</th><th>Avg RSP</th><th>Max RSP</th><th>Min Industry</th><th>Avg Industry</th><th>Max Industry</th></tr></thead><tbody>${productTempRows.map(row => `<tr><td>${escapeHtml(row.product)}</td><td>${escapeHtml(row.temperature)}</td><td>${money(row.minRsp, currency)}</td><td>${money(row.avgRsp, currency)}</td><td>${money(row.maxRsp, currency)}</td><td>${money(row.minInd, currency)}</td><td>${money(row.avgInd, currency)}</td><td>${money(row.maxInd, currency)}</td></tr>`).join("") || `<tr><td colspan="8">No rows available</td></tr>`}</tbody></table>`;
  ...
  const similarProductRows = productTempRows.map(row => `<tr><td>${escapeHtml(row.product)}</td><td>${escapeHtml(row.temperature)}</td><td>${money(row.avgRsp, currency)}</td><td>${money(row.avgInd, currency)}</td><td><div class="report-bar"><span style="width:${Math.min(100, (row.avgRsp / maxChartAvg) * 100)}%"></span></div></td></tr>`).join("");
  const productPriorityRows = [...productTempRows].sort((a, b) => ((b.avgInd - b.avgRsp) / (b.avgInd || 1)) - ((a.avgInd - a.avgRsp) / (a.avgInd || 1))).slice(0, 5).map(row => {
    const marginPct = row.avgInd ? ((row.avgInd - row.avgRsp) / row.avgInd) * 100 : 0;
    return `<tr><td>${escapeHtml(row.product)}</td><td>${escapeHtml(row.temperature)}</td><td>${money(row.avgRsp, currency)}</td><td>${money(row.avgInd, currency)}</td><td>${Number(marginPct).toFixed(1)}%</td></tr>`;
  }).join("");
  const retailerSpreadRows = retailerRows.slice(0, 5).map(row => `<tr><td>${escapeHtml(row.retailer)}</td><td>${money(row.avgRsp, currency)}</td><td>${money(row.avgIndustry, currency)}</td><td>${money(row.spread, currency)}</td><td>${Number(row.marginPct).toFixed(1)}%</td></tr>`).join("");
  const outlierRows = [...data].sort((a, b) => {
    const gapA = number(a.industryPrice) ? ((number(a.priceKg) - number(a.industryPrice)) / number(a.industryPrice)) * 100 : 0;
    const gapB = number(b.industryPrice) ? ((number(b.priceKg) - number(b.industryPrice)) / number(b.industryPrice)) * 100 : 0;
    return Math.abs(gapB) - Math.abs(gapA);
  }).slice(0, 5).map(row => {
    const gap = number(row.industryPrice) ? ((number(row.priceKg) - number(row.industryPrice)) / number(row.industryPrice)) * 100 : 0;
