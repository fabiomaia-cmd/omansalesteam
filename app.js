...
function recordFromForm() {
  ...
  return { id: editingId || crypto.randomUUID(), date: $("date").value, manufacturer: $("manufacturer").value.trim(), originCountry: selectedOriginCountry(), collector: $("collector").value.trim(), channel: $("channel").value, city: selectedCity(), country: selectedCountry(), currency, store: $("store").value.trim(), protein: $("protein").value, temperature: $("temperature").value, product: $("product").value, subProduct: $("subProduct").value === "__custom" ? $("subProductCustom").value.trim() : $("subProduct").value, packagingType: $("packagingType").value === "__custom" ? $("packagingTypeCustom").value.trim() : $("packagingType").value, weight, packPrice, priceKg, priceUsdKg, margin, industryPrice: manual || priceUsdKg * (1 - margin / 100), manualIndustryPrice: Boolean(manual), promotion: $("promotion").checked, fullPrice: $("promotion").checked ? number($("fullPrice").value) : "", promoPack: $("promoPack").checked, comboQty: $("promoPack").checked ? number($("comboQty").value) : "", comboUnitWeight: $("promoPack").checked ? number($("comboUnitWeight").value) : "", comboTotalPrice: $("promoPack").checked ? packPrice : "" };
}
