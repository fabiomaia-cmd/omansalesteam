...
let records = JSON.parse(localStorage.getItem(STORAGE_KEY) || "[]").map(record => Object.fromEntries(Object.entries(record).map(([key, value]) => [key, typeof value === "string" && LEGACY[value] ? LEGACY[value] : value])));
...
function populateSubProducts(selected = "") { const protein = $("protein").value, product = $("product").value, options = SUBPRODUCTS[product]?.[protein] || []; $("subProduct").innerHTML = `<option value="">Not applicable</option>${options.map(item => `<option ${item === selected ? "selected" : ""}>${item}</option>`).join("")}`; }
...
function openDialog(record = null, photo = null, transcript = "") {
  ...
  if (record) {
    ...
    for (const key of ["collector", "channel", "store", "protein", "temperature", "weight", "packPrice", "priceKg", "priceUsdKg", "margin", "industryPrice", "currency", "fullPrice", "subProduct", "comboQty", "comboUnitWeight", "comboTotalPrice"]) if ($(key)) $(key).value = record[key] ?? "";
...
function recordFromForm() {
  ...
  return { id: editingId || crypto.randomUUID(), date: $("date").value, collector: $("collector").value.trim(), channel: $("channel").value, city: selectedCity(), country: selectedCountry(), currency, store: $("store").value.trim(), protein: $("protein").value, temperature: $("temperature").value, product: $("product").value, subProduct: $("subProduct").value, weight, packPrice, priceKg, priceUsdKg: priceKg / fx, margin, industryPrice: manual || priceKg * (1 - margin / 100), manualIndustryPrice: Boolean(manual), promotion: $("promotion").checked, fullPrice: $("promotion").checked ? number($("fullPrice").value) : "", promoPack: $("promoPack").checked, comboQty: $("promoPack").checked ? number($("comboQty").value) : "", comboUnitWeight: $("promoPack").checked ? number($("comboUnitWeight").value) : "", comboTotalPrice: $("promoPack").checked ? number($("comboTotalPrice").value) : "" };
}
...
function render() { refreshFilterOptions(); populateRetailerSuggestions(); const data = filteredRecords(); $("recordCount").textContent = records.length; $("visibleCount").textContent = data.length; $("emptyState").style.display = data.length ? "none" : "block"; renderMetrics(data); $("priceTable").innerHTML = data.map(r => `<tr><td><input type="checkbox" class="row-check" data-id="${r.id}"></td><td>${r.date || "—"}</td><td><b>${escapeHtml(r.store)}</b><span class="subtle">${escapeHtml(r.collector || "")} · ${escapeHtml(r.channel || "")}</span></td><td><span class="product-name">${escapeHtml(r.protein)}</span><span class="subtle">${escapeHtml(r.product)}${r.subProduct ? ` · ${escapeHtml(r.subProduct)}` : ""}${r.promotion ? " · Promotion" : ""}</span></td><td><span class="tag ${r.temperature === "Frozen" ? "cold" : ""}">${r.temperature}</span></td><td class="price">${money(r.priceKg, r.currency || "USD")}</td><td class="price teal-price">${money(r.priceUsdKg)}</td><td class="price teal-price">${money(r.industryPrice, r.currency || "USD")}</td><td><button class="row-action edit-row" data-id="${r.id}" title="Edit">✎</button></td></tr>`).join(""); renderInsights(data); $("deleteSelected").style.display = data.length ? "block" : "none"; }
...
$("manualBtn").onclick = () => openDialog(); $("closeDialog").onclick = closeDialog; $("cancelDialog").onclick = closeDialog; $("country").onchange = () => populateCountryFields(); $("city").onchange = () => { $("cityCustom").hidden = $("city").value !== "__custom"; $("cityCustom").required = $("city").value === "__custom"; }; $("protein").onchange = () => populateProducts(); $("product").onchange = () => populateSubProducts(); ["weight", "packPrice", "priceKg", "industryPrice", "margin", "currency", "comboQty", "comboUnitWeight", "comboTotalPrice"].forEach(id => $(id).oninput = updateCalculation); $("promotion").onchange = toggleConditionalFields; $("promoPack").onchange = toggleConditionalFields; $("audioBtn").onclick = openTranscriptionTab;
$("entryForm").onsubmit = event => { event.preventDefault(); const item = recordFromForm(), index = records.findIndex(r => r.id === item.id); localStorage.setItem(COLLECTOR_KEY, item.collector); if (index >= 0) records[index] = item; else records.unshift(item); save(); closeDialog(); showToast(index >= 0 ? "Collection updated." : "Collection saved."); };
document.querySelectorAll(".filters input,.filters select").forEach(el => el.addEventListener("change", render)); $("clearFilters").onclick = () => { document.querySelectorAll(".filters input,.filters select").forEach(el => el.value = ""); render(); }; $("priceTable").onclick = event => { const button = event.target.closest(".edit-row"); if (button) openDialog(records.find(r => r.id === button.dataset.id)); }; $("selectAll").onchange = event => document.querySelectorAll(".row-check").forEach(c => c.checked = event.target.checked); $("deleteSelected").onclick = () => { const ids = [...document.querySelectorAll(".row-check:checked")].map(c => c.dataset.id); if (ids.length && confirm(`Delete ${ids.length} collection(s)?`)) { records = records.filter(r => !ids.includes(r.id)); save(); showToast("Collections deleted."); } }; $("exportBtn").onclick = exportCsv; $("importInput").onchange = event => { const file = event.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => { const imported = parseCsv(reader.result); records = [...imported, ...records.filter(old => !imported.some(item => item.id === old.id))]; save(); showToast(`${imported.length} record(s) imported.`); }; reader.readAsText(file, "UTF-8"); event.target.value = ""; }; $("photoBtn").onclick = () => $("photoInput").click(); $("photoInput").onchange = event => { const file = event.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => openDialog(null, reader.result); reader.readAsDataURL(file); event.target.value = ""; };
