...
function populateProducts(selected = "") { const options = PRODUCTS[$("protein").value] || []; $("product").innerHTML = `<option value="">Select</option>${options.map(item => `<option ${item === selected ? "selected" : ""}>${item}</option>`).join("")}`; populateSubProducts(); populatePackagingTypes(); }
...
function togglePackagingCustom() { const custom = $("packagingType").value === "__custom"; $("packagingTypeCustom").hidden = !custom; $("packagingTypeCustom").required = custom; }
function suggestedRetailMargin(protein = $("protein").value) { return ["Beef", "Lamb"].includes(protein) ? 30 : protein === "Chicken" ? 20 : 20; }
...
function openDialog(record = null, photo = null, transcript = "") {
  ...
  if (record) {
    ...
    $("packagingType").value = record.packagingType || ""; $("packagingTypeCustom").value = record.packagingType && !PACKAGING_TYPES[$("product").value]?.includes(record.packagingType) ? record.packagingType : ""; $("packagingTypeCustom").hidden = !($("packagingType").value === "__custom"); $("packagingTypeCustom").required = $("packagingType").value === "__custom";
...
function updateCalculation() {
  const weight = calculatedWeight(), packPrice = calculatedPackPrice(), marketPrice = weight && packPrice ? packPrice / weight : 0, fx = FX_TO_USD[$("currency").value] || 1, marketPriceUsd = marketPrice / fx, manual = number($("industryPrice").value), margin = number($("margin").value), vat = countryVatRate(), industryUsd = manual || marketPriceUsd * (1 - margin / 100) * (1 + vat / 100);
...
function recordFromForm() {
  const weight = calculatedWeight(), packPrice = calculatedPackPrice(), priceKg = weight && packPrice ? packPrice / weight : 0, currency = $("currency").value, fx = FX_TO_USD[currency] || 1, priceUsdKg = priceKg / fx, manual = number($("industryPrice").value), margin = number($("margin").value), vat = countryVatRate();
  return { id: editingId || crypto.randomUUID(), date: $("date").value, manufacturer: $("manufacturer").value.trim(), originCountry: selectedOriginCountry(), collector: $("collector").value.trim(), channel: $("channel").value, city: selectedCity(), country: selectedCountry(), currency, store: $("store").value.trim(), protein: $("protein").value, temperature: $("temperature").value, product: $("product").value, subProduct: $("subProduct").value === "__custom" ? $("subProductCustom").value.trim() : $("subProduct").value, packagingType: $("packagingType").value === "__custom" ? $("packagingTypeCustom").value.trim() : $("packagingType").value, weight, packPrice, priceKg, priceUsdKg, margin, industryPrice: manual || priceUsdKg * (1 - margin / 100) * (1 + vat / 100), manualIndustryPrice: Boolean(manual), promotion: $("promotion").checked, fullPrice: $("promotion").checked ? number($("fullPrice").value) : "", promoPack: $("promoPack").checked, comboQty: $("promoPack").checked ? number($("comboQty").value) : "", comboUnitWeight: $("promoPack").checked ? number($("comboUnitWeight").value) : "", comboTotalPrice: $("promoPack").checked ? packPrice : "" };
}
